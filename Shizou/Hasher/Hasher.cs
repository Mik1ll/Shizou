...
namespace Shizou.Hasher
{
    ...
    {
        public static (string ed2k, string crc, string sha1, string md5) GetHash(string filepath, bool useTask = false)
        {
            var hasher = new RHasher(HashIds.Ed2K | HashIds.Crc32 | HashIds.Sha1 | HashIds.Md5);
            if (useTask)
                hasher.UpdateFileWithTask(filepath);
            else
                hasher.UpdateFile(filepath);
            hasher.Finish();
            return (hasher.ToString(HashIds.Ed2K), hasher.ToString(HashIds.Crc32), hasher.ToString(HashIds.Sha1), hasher.ToString(HashIds.Md5));
        }
        ...
        private sealed class RHasher
        {
            ...
            public static string GetHashForFile(string filePath, HashIds id)
            {
                return new RHasher(id).UpdateFile(filePath).Finish().ToString();
            }
        }
